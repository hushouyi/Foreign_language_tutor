...
点击屏幕左侧绿色 ▶ 按钮打开侧边栏：
- **LLM 引擎** — 切换 DeepSeek API ↔ 本地 Ollama 模型
- **联网搜索** — 开启/关闭联网实时搜索（需先启动 SearXNG Docker 容器）
- **服务状态** — 查看本地 Ollama、SearXNG 等服务运行状态
...
需要联网搜索或本地 LLM 时，先安装 [Docker Desktop](https://www.docker.com/products/docker-desktop/)，然后：

```bash
# 启动所有本地服务（SearXNG + Ollama 等）
docker compose up -d
...
启动后侧边栏「联网搜索」选 SearXNG 即可。AI 会自动判断是否需要搜索实时信息（新闻、天气、事件等），搜索结果会自然融入回答中。
