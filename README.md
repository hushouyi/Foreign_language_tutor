...
基于 **DeepSeek V4 Flash + edge-tts** 的多语种 AI 语言练习伙伴。键盘输入文本，AI 用真人语音回复并附中文翻译。**默认使用 Web 界面**（微信聊天风格），也保留 CLI 终端模式。
...
Rich 面板逐段显示，语音在后台连续播放：
- 回复分段显示（每段间隔约 2 秒），音频在后台连续播放
- "Alice thinking..." / "Alice speaking..." 状态提示
- 可以边听语音边打字，互不干扰
...
```bash
# Web 模式（默认）：启动 Flask 服务器，自动打开浏览器
python main.py

# 指定语种启动
python main.py --lang japanese

# CLI 终端模式（保留的旧版界面）
python main.py --cli
python main.py --cli --lang japanese
```
...
```
├── main.py                 # 入口；默认=Flask Web 服务器，--cli=终端模式
├── web_server.py           # Flask 服务器：NDJSON 流式推送、TTS 音频提供
├── templates/
